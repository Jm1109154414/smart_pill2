/*
 * PillMate / Smart Pill ESP32 Client
 *
 * Este sketch conecta tu pastillero ESP32 con el backend (Supabase).
 * Asegúrate de configurar correctamente config.h antes de subirlo.
 */
...
// URLs de las Edge Functions (usa la URL de tu proyecto Supabase)
...
Schedule    schedules[15]; // Máx 3 horarios por compartimento
...
const unsigned long CONFIG_INTERVAL = 300000; // 5 min
const unsigned long POLL_INTERVAL   = 30000;  // 30 s
...
void setup() {
  ...
  Serial.println();
  Serial.println("=== Smart Pill ESP32 Client ===");
  ...
  // Obtener configuración inicial desde Supabase
  fetchDeviceConfig();

  // Configurar pines (ajusta según tu hardware real)
...
void loop() {
  ...
  // Polling de comandos cada 30 s
  if (now - lastCommandPoll > POLL_INTERVAL) {
    lastCommandPoll = now;
    pollCommands();
  }

  // Refrescar configuración cada 5 min
  if (now - lastConfigFetch > CONFIG_INTERVAL) {
    lastConfigFetch = now;
    fetchDeviceConfig();
  }

  // Aquí después irán tus checkSchedules(), lectura de sensores, etc.
  delay(1000);
}
...
void fetchDeviceConfig() {
  ...
  Serial.println("Obteniendo configuración del dispositivo...");
    ...
    if (!error) {
      ...
      if (commands.size() > 0) {
        ...
        for (JsonObject cmd : commands) {
          ...
          bool success = processCommand(type, p);
...
bool processCommand(String type, JsonObject payload) {
  if (type == "snooze") {
    ...
    // Aquí iría la lógica real de snooze
    return true;
...
void activateLocalAlarm() {
  // Aquí luego metes buzzer/servo/etc.
  Serial.println("🔔 Alarma local activada (LED_BUILTIN)");
